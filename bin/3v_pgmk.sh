...
make_dir_vars() {
	...
	readonly bin_dir="${PGMK_HOME}/bin"
	readonly dumps_dir="${PGMK_HOME}/dumps"
	readonly passkeys_dir="${PGMK_HOME}/passkeys"

	# variables of file names LOCAL
	readonly exec_file_name="pgmk.sh"
	readonly exec_file="${bin_dir}/${exec_file_name}"

	readonly dump_file_name="${REMOTE_DB_NAME}_${REMOTE_IP}.dump"
	readonly dump_file="${dumps_dir}/${dump_file_name}"

	readonly dump_logs_file_name="dumplogs.txt"
	readonly dump_logs_file="${dumps_dir}/${dump_logs_file_name}"

	readonly postgres_pass_file_name="postgres_pass"
	readonly postgres_pass_file="${passkeys_dir}/${postgres_pass_file_name}"

	readonly ssh_pass_file_name="ssh_pass"
	readonly ssh_pass_file="${passkeys_dir}/${ssh_pass_file_name}"


	# variables of dir and files REMOTE
	# readonly remote_HOME="/home/${SSH_USER_NAME}/.pgmk"
	readonly remote_HOME="/opt/pgmk"
	readonly remote_dump_file="${remote_HOME}/dumps/pgmk.dump"
...
version() {
	cat 1>&2 <<EOF
pgmk-init 0.0.3 (Mon Jul 20 12:14:47 IST 2019)
EOF
}
invalid_arg() {
	cat 1>&2 <<EOF
