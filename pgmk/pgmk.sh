...
min_EOF
	...
	## define an array ##
	arrayname=( SSH_USER_NAME REMOTE_IP REMOTE_DB_NAME LOCAL_DB_NAME LOCAL_DB_USER db_port )
...
usage() {
	...
	USAGE:
		...
		pgmk --connecttodb		:	Connects to local machines pgmk database
...
connect_to_local_pgmk_db() {
	say "connect_to_local_pgmk_db Invoked()--------------------"
	check_func
	if [ $local_DB_exists = "yes" ]
	then
		say "local_DB_exists=${local_DB_exists}, connecting....."
		psql -h 127.0.0.1 -p 5432 -U postgres  ${LOCAL_DB_NAME}
	else
		echo ""
		echo "${LOCAL_DB_NAME} does not exist"
	fi
}
for arg in "$@"
do	
